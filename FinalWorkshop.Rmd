...
Model 1
```{r}
#Model 1: Earnings response coefficient
...
# sorted2019 = paneldata_2019[order(paneldata_2019$fscore,decreasing = TRUE),]
...
After the analysis of these companies and the characteristics meant to reach by this workshop, we can observe a real application of Econometrics as a helper tool to create informed portfolio investments. Indicators like F-Score or signals, like the ones applied for inventory, are incredible resources to formalize fundamental analysis over companies and to obtain a broader perspective over their current status.
